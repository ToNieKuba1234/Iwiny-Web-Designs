...
export default function Technologies() {
  return (
    <section className="bg-gradient-to-b bg-[#1c1c2b] w-full text-white h-auto flex flex-col justify-between items-center gap-y-20">
      ...
      <ul className="grid w-auto grid-cols-2 gap-6 lg:grid-cols-3">
        {technologies.map(({ src, alt, link }) => (
          <li key={alt} className="flex flex-col group">
            <a href={link} title={alt} target="_blank" rel="noopener noreferrer" className="w-full h-full">
      ...
      <div className="bottom-0 left-0 w-[120%] h-[150px] bg-[#b0b0b0] rounded-t-[100%] -ml-[10%] mt-16"></div>
    </section>
  );
}